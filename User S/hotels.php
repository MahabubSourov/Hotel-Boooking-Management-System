...
<section class="section-sm">
    <div class="container">
        <!-- Filter Section -->
        <div
            style="background: white; padding: 1.5rem; border-radius: 1rem; box-shadow: var(--shadow-md); margin-bottom: 2rem;">
            ...
            <form method="GET"
                ...
                <div class="form-group">
                    ...
                    <select name="type" id="type" class="form-select">
                        ...
                        <option value="international" <?php echo $type === 'international' ? 'selected' : ''; ?>>
                ...
                <div class="form-group">
                    ...
                    <input type="number" name="min_price" id="minPrice" class="form-input" placeholder="Min"
                        value="<?php echo $minPrice; ?>">
                </div>

                <div class="form-group">
                    <label class="form-label">Max Price</label>
                    <input type="number" name="max_price" id="maxPrice" class="form-input" placeholder="Max"
                ...
                <?php if ($checkIn): ?><input type="hidden" name="check_in"
                        value="<?php echo $checkIn; ?>"><?php endif; ?>
                <?php if ($checkOut): ?><input type="hidden" name="check_out"
                        value="<?php echo $checkOut; ?>"><?php endif; ?>
        ...
        <?php if ($hotels->num_rows > 0): ?>
            <?php while ($hotel = $hotels->fetch_assoc()):
                ...
                if ($minPrice && $hotelMinPrice < $minPrice)
                    continue;
                if ($maxPrice && $hotelMinPrice > $maxPrice)
                    continue;
                if ($minRating && ($hotel['avg_rating'] ?? 0) < $minRating)
                    continue;
                ?>
                <div class="hotel-card">
                    <img src="<?php echo fixImagePath($hotel['image']) ?: 'https://via.placeholder.com/300x250?text=' . urlencode($hotel['name']); ?>"
                        alt="<?php echo htmlspecialchars($hotel['name']); ?>" class="hotel-card-image">
